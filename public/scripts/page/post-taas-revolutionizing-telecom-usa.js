/* Custom site scripts extracted from the WordPress page "post-taas-revolutionizing-telecom-usa".
...
document.addEventListener("DOMContentLoaded", function () {
    const checkboxes = document.querySelectorAll(".taas-checklist input[type='checkbox']");

    checkboxes.forEach((checkbox) => {
      // Load state from localStorage
      const saved = localStorage.getItem("taas_" + checkbox.id);
      if (saved !== null) {
      ...
      checkbox.addEventListener("change", () => {
        localStorage.setItem("taas_" + checkbox.id, checkbox.checked);
      });
    });
  });
})();
