/* Custom site scripts extracted from the WordPress page "post-launch-us-mvno-now".
...
document.addEventListener("DOMContentLoaded", function () {
    const checkboxes = document.querySelectorAll(".mvnx-checklist input[type='checkbox']");

    checkboxes.forEach((checkbox) => {
      const saved = localStorage.getItem("mvnx_" + checkbox.id);
      if (saved !== null) {
        checkbox.checked = saved === "true";
      }

      checkbox.addEventListener("change", () => {
        localStorage.setItem("mvnx_" + checkbox.id, checkbox.checked);
      });
    });
  });
})();
