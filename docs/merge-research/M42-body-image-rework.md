...
DROP three text cards and preserve their exact checks in a fourth HTML table. The new illustration shows separate observations converging into a campaign record, with the changed profile nested inside the device.

Verification at 1024px: Inspected at 1024px: labels readable, orange field, visible object relationships and depth.

Replacement: `/blog-media/esim-orchestration/profile-network-application-observations.png`. Original PNG and WebP deleted.

1600×900 PNG: 760,570 bytes. WebP: 164,824 bytes.

## Preservation and gate

Only replacement tables and figure metadata change. Original prose, existing tables and published images are unchanged.  All 2 published body figures retain their exact markup and srcset. Every protected hero/other local asset retains its original SHA-256. Information gain stays 7. `npm run merge:qa -- M42`: PASS.
